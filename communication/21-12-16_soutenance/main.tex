...
\begin{frame}{Convergence when $T_c\to 0$ for 1dx-1dv model}
  \begin{figure}
    ...
    \caption{Initial condition (left), solution at $t=200$ of the full kinetic model with $T_c=0.05$ (middle) and the hybrid model (right).}
  \end{figure}
...
% -------------------------------------------------------------------
% --- APPLICATION TO HYBRID VLASOV-MAXWELL MODEL --------------------
% -------------------------------------------------------------------
\changecolor{sec3}
...
\begin{frame}{Numerical results}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \item Modified Lawson method with Padé:\begin{itemize}
...
\begin{frame}{Numerical results: splitting vs Lawson}
  ...
  \only<3>{
    \begin{table}[h]
      ...
      \end{tabular}
      \caption{Computational time for the different methods, on mesh $N_z \times N_{v_x} \times N_{v_y} \times N_{v_z}=27\times32\times32\times41$ and time step $\Delta t = 0.05$.}
    \end{table}
  }
\end{frame}
...
\begin{frame}{Numerical results: Padé-Lawson adaptive time step}
  \only<1>{
    \begin{figure}
      ...
      \vspace{-0.25cm}
      \caption{Energies evolution, Lawson with Padé approximation, $\Delta t^n$}
    \end{figure}
  }
  \only<2>{
    \begin{figure}
      \includegraphics[width=\textwidth]{img/dt_ldppt}
      \caption{Time step evolution, classic Lawson (Maxwell outside) and approximate Lawson with Padé or Strang approximation, $\Delta t_n$}
    \end{figure}
    \begin{table}[h]
      \centering
...
\end{frame}
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{l|r}
      ...
      LDP4(3) - Strang     &                $04\,\text{h}\ 42\,\text{min}\ 25\,\text{s}$ \\
    \end{tabular}
    \caption{Computational time for the different methods, on mesh $N_z \times N_{v_x} \times N_{v_y} \times N_{v_z}=27\times32\times32\times41$ and time step $\Delta t = 0.05$ (initial time step for adaptive time step strategy).}
  \end{table}
\end{frame}
