...
\Josselin{Je ne suis pas certain de la notation $\mathfrak{exp}$, on comprend bien que ça a un lien avec la fonction $\exp$, on n'indique pas d'ordre (en même temps Taylor ne prend qu'un indique et Padé 2, chose que je résume par un $\order{\Delta t^{r+1}}$), et ça évite de dupliquer les expressions pour Taylor puis pour Padé.}
...
\Josselin{Si j'ai le temps en rentrant de vacances, mettre ici une simu dans le cas rotation où Taylor expose mais pas Padé.}
