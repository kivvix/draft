...
Nous effectuons le même type d'analyse sur l'énergie électrique, à partir des données des simulations précédentes, en sachant qu'il n'existe pas de résultat théorique sur sa convergence. L'énergie électrique pour les différents choix de $T_c$ est représentée sur la figure~\ref{fig:limit:ee} (gauche), cette figure illustre mieux la nécessité d'effectuer une interpolation pour extraire les données. Une convergence est observée sur la figure~\ref{fig:limit:ee} (droite).
\begin{figure}[h]
  \centering
...
\Josselin{J'ai passé un peu plus de temps que prévu à refaire ces figures mais au moins j'ai pu refaire ces figures à partir des \emph{runs} que j'avais déjà. La suite est plus simple, ce sont à peu de choses près les mêmes figures (en moins jolies) que celles du papier, donc c'est de la reprise des figures et améliorer le blabla dans les paragraphes. Et il y a même plus de choses dans le papier sur cette partie (ordre, temps de calcul, comparatifs plus précis de Lawson et Suzuki, test de CFL, etc.). Bref je laisse cette seconde partie à plus tard.}
